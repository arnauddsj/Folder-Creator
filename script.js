...
Object.keys(newFileList).forEach(key => {
    ...
    // Built origin path
    const originPath = path.join(newFileList[key].originpath.concat("\\", fileNameExt))
    //Build dest path
    const imageDestPath = path.join(newFileList[key].originpath, folderName, subFolderName, fileNameExt)

    fs.renameSync(originPath, imageDestPath, (err) => { if (err) throw err; });
});